...
/*   Updated: 2025/02/04 17:53:20 by alacroix         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	create_head_env_lst(char *name, char *value, t_env **env)
{
	*env = malloc(sizeof(t_env));
	if (!*env)
		return (error(MEM, "create_head_env_lst"));
	(*env)->name = name;
	(*env)->value = value;
	(*env)->next = NULL;
}

void	create_node_env_lst(char *name, char *value, t_env **env)
{
	t_env	*temp;
	t_env	*new;
	...
	if (!new)
		return (error(MEM, "create_node_env_lst"));
	new->name = name;
	new->value = value;
	new->next = NULL;
	while (temp->next)
