...
/*   Updated: 2025/02/18 11:39:01 by kyang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

# include "minishell.h"

static void	print_syntax_error(char *token_value)
{
...
static void	check_redirect(t_token **current)
{
...
static void	check_operator(t_token **current)
{
...
static void	check_current(t_token **current, t_token **last)
{
	while (*current)
	{
		...
		((*current)->token_type != TOKEN_PIPE && (*current)->token_type != TOKEN_AND && (*current)->token_type != TOKEN_OR))
			print_syntax_error((*current)->next->next->value);
		*last = *current;
		(*current) = (*current)->next;
	}
}
